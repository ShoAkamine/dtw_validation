...
df_dtw_f2f = read_csv("data/dtw_distance_f2f.csv") %>% 
  select(comparison_id, pair, target, average_distance, average_distance_xyz,
...
mean_distance_by_pp_f2f = df_dtw_f2f %>% 
  group_by(pair) %>%
  summarize(mean_distance = mean(average_distance_xyz)) %>%
  ungroup()

mean_distance_f2f_smop = df_dtw_f2f %>%
  group_by(n_features_smop) %>%
  summarize(mean_distance = mean(average_distance_xyz),
            sd = sd(average_distance_xyz),
            se = std.error(average_distance_xyz),
            count = n(),
...
dis_f2f_feature = df_dtw_f2f_long %>%
  ggplot(aes(x = feature, y = average_distance_xyz, 
...
```{r, include = FALSE}
pdf(file="figures/distance_f2f_feature.pdf", width=8, height=4)
dis_f2f_feature
dev.off()
```
...
```{r, include = FALSE}
pdf(file="figures/distance_f2f_n_features.pdf", width=8, height=4)
dis_f2f_n_features
dev.off()
```
...
pdf(file="figures/distance_f2f_combined.pdf", width=10, height=5)
combined_p
dev.off()
```
...
y = df_dtw_f2f$average_distance_xyz
...
```{r}
lmer_n_features = lmer(average_distance_xyz ~ n_features_smop + 
                         (1|pair) + (1|target), 
                       data = df_dtw_f2f)
...
### all features
lmer_feature = lmer(average_distance_xyz ~ 
...
```{r, include = FALSE}
pdf(file="figures/distance_zoom_feature.pdf", width=8, height=6)
dis_zoom_feature
dev.off()
```
...
dis_zoom_n_features = df_dtw_zoom %>%
  ggplot(aes(x = n_features, y = average_distance_xyz, 
             group = n_features)) +
  ggdist::stat_halfeye(adjust = 1, width = 0.8, .width = 0,
                       point_color = NA, alpha = 0.5) +
...
```{r, include = FALSE}
pdf(file="figures/distance_zoom_n_features.pdf", width=8, height=4)
ggExtra::ggMarginal(dis_zoom_n_features, fill = "lightblue")
dev.off()
```
...
pdf(file="figures/distance_zoom_combined_bin.pdf", width=10, height=5)
combined_p
dev.off()
```
...
The correlation between the total similarity score and the normalized DTW distance is significant (r = -0.391, p < 0.001), suggesting that the more similar features two gestures have, the smaller the distance is.
...
#### Number of features
```{r}
lmer_zoom_n_features = lmer(average_distance_xyz ~ 
                              n_features +
                              (1|pair) + (1|target),
                            data = df_dtw_zoom)
...
```{r, include = FALSE}
pdf(file="figures/distance_zoom_v2_feature.pdf", width=8, height=6)
dis_zoom_v2_feature
dev.off()
```
...
```{r, include = FALSE}
pdf(file="figures/distance_zoom_v2_n_features.pdf", width=8, height=4)
ggExtra::ggMarginal(dis_zoom_v2_n_features, fill = "lightblue")
dev.off()
```
...
pdf(file="figures/distance_zoom_v2_combined_bin.pdf", width=10, height=5)
combined_p
dev.off()
```
...
The modifed DTW pipeline results in a mush stronger negative correlation (r = -0.561, p < 0.001) between the number of similar features and the normalized DTW distance compared to the original pipeline (r = -0.391). This suggests that the modified DTW pipeline is more sensitive to the similarity of gestures.
